...
public class SA1 {
  ...
  // calculates acceptance probability of given distances(energy) with respect to
  // given temperature
  ...
  /*
   * simulating annealing algorithm starts from a max temperature which is
   * predefined gets a random node as current node calculates cycle rate and with
   * this cycle rate calculates cooling rate while cooling for cycle rate
   * iteration: get for each cycle rate gets a neuron as next calculates accetance
   * probability of distance of the next and current node to given coordinate if
   * find a better result keeps it increases step number and cools the max
   * temperature return index of the best neuron
   */
