...
public class Visual extends GraphicsProgram {
  ...
  private double toRealX(double x) {
    return (width - MARGIN) * x + MARGIN / 2;
  }

  private double toRealY(double y) {
    return (height - MARGIN) * y + MARGIN / 2;
  }
  ...
  private void drawMapLine(int id, Neuron firstInstance, Neuron nextInstance) {
    ...
    GLine line = new GLine(toRealX(firstInstance.getWX()), toRealY(firstInstance.getWY()),
  ...
  private void addNode(Node instant) {
    GOval point = new GOval(toRealX(instant.getX()), toRealY(instant.getY()), DOTSIZE, DOTSIZE);
